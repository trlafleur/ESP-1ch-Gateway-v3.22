...
#define VERSION " ! Ver: 3.2.2a, 20Dec2016"    // <--------- trl
...
#include "gBase64.h"              // https://github.com/adamvr/arduino-base64 (I changed the name)  //  <----------- trl
...
int ssPin = 2;									// GPIO16, D0   // TRL <------------------ pin changes
...
void sendstat() {
  ...
  sprintf(stat_timestamp, "%04d-%02d-%02d %02d:%02d:%02d PST", year(),month(),day(),hour(),minute(),second());    // <--------------trl
	yield();
	
	
	ftoa(lon,clon,4);										// As Arduino CANNOT prints floats
  ftoa(lat,clat,4);                    // Convert lat to char array with 4 decimals    // <--------------trl
